...
import { ArrowLeft, Activity, DollarSign, Users, FileText } from "lucide-react";
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <Button asChild><Link to="/research"><ArrowLeft className="mr-2 h-4 w-4" /> Voltar para Pesquisa</Link></Button>
        </div>
      </div>
    );
  }
        ...
        <div className="absolute bottom-0 left-0 right-0 pb-10 pt-20">
          <div className="container mx-auto px-4">
            <Link to="/research" className="inline-flex items-center text-primary-foreground/70 hover:text-primary-foreground text-sm mb-4 transition-colors">
              <ArrowLeft className="mr-1 h-4 w-4" /> Voltar para Pesquisa
            </Link>
            ...
            <div className="space-y-6">
              <FadeUp delay={0.1}>
                <div className="bg-card rounded-xl p-6 shadow-card">
                  <h3 className="font-bold text-foreground mb-4">Detalhes do Projeto</h3>
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      <Activity className="h-4 w-4 text-primary flex-shrink-0" />
                      <div>
                        <p className="text-xs text-muted-foreground">Status</p>
                        <p className="text-sm font-medium text-foreground">{statusLabels[project.status] ?? project.status}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
              ...
              <FadeUp delay={0.2}>
                <div className="bg-card rounded-xl p-6 shadow-card">
                  <h3 className="font-bold text-foreground mb-4">Equipe</h3>
              ...
              {relatedPubs.length > 0 && (
                <FadeUp delay={0.3}>
                  <div className="bg-card rounded-xl p-6 shadow-card">
                    <h3 className="font-bold text-foreground mb-4">Publicações Relacionadas</h3>
