    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <Button asChild><Link to="/news"><ArrowLeft className="mr-2 h-4 w-4" /> Voltar para Notícias</Link></Button>
        </div>
      </div>
    );
  }
      ...
      <section className="bg-hero py-16 lg:py-20">
        <div className="container mx-auto px-4">
          <Link to="/news" className="inline-flex items-center text-primary-foreground/70 hover:text-primary-foreground text-sm mb-4 transition-colors">
            <ArrowLeft className="mr-1 h-4 w-4" /> Voltar para Notícias
          </Link>
          <div className="flex items-center gap-3 mb-4">
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${badgeColors[item.category]}`}>{categoryLabels[item.category] ?? item.category}</span>
          </div>
        ...
        <div className="container mx-auto px-4 max-w-3xl">
          <FadeUp>
            <div className="bg-card rounded-xl p-8 lg:p-10 shadow-card">
              {item.fullText.split("\n\n").map((paragraph, i) => {
                const labelUrl = paragraph.match(/^\*\*(.*?)\*\*\s*\n\s*(https?:\/\/\S+)\s*$/);
                if (labelUrl) {
                  const label = labelUrl[1].replace(/:\s*$/, "").trim();
                  const url = labelUrl[2];
                  return (
